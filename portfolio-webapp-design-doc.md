...
- 목표 투자금액 기준 리밸런싱 수량 및 현금 비중 계산
- 종목 추가·수정·삭제, 중복 티커 및 잘못된 숫자 검증
- 자산배분 도넛 차트와 백테스트 라인 차트(외부 차트 라이브러리 없이 동작)
- 변동성, MDD, 누적수익률, CAGR, 비교지수 대비 베타 계산
- 브라우저 자동 저장, JSON 백업·복원, CSV 내보내기
- 모바일 대응, 키보드 입력, 폼 라벨 등 기본 접근성

이 문서 아래의 React + API 구조는 다음 단계에서 실시간 시세·검색·뉴스·재무정보를 붙이기 위한 확장 설계입니다. 현재 단일 HTML 버전에는 API 키가 필요한 기능을 억지로 포함하지 않았습니다.
