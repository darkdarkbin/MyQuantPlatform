...
def build_one(client: NasdaqClient, symbol: str) -> dict[str, Any]:
    ...
    # Nasdaq's own data only recognizes class shares with a hyphen (BRK-B), but
    # people commonly type the period form (BRK.B). Try the literal symbol first,
    # then fall back to the hyphen form so a period doesn't hard-fail the request.
    candidates = [symbol]
    if "." in symbol:
        hyphenated = symbol.replace(".", "-")
    ...
    for candidate in candidates:
        for builder in (stock_research, etf_research):
            try:
                return builder(client, candidate)
...
def parse_requested_tickers(args: argparse.Namespace) -> tuple[list[str], set[str]]:
    """Return (symbols, explicitly_requested).

    `explicitly_requested` only covers tickers a person asked for directly
    (via --ticker or a "[data] TICKER" issue title) -- NOT the bulk
    --defaults/--refresh-cached lists, where an occasional upstream miss is
    expected and should not fail the whole job.
    """
    explicit_values: list[str] = []
    for raw in args.ticker or []:
        explicit_values.extend(re.split(r"[,\s]+", raw))
    if args.issue_title:
        match = re.search(r"\[DATA\]\s*([A-Z0-9.\-^, ]+)", args.issue_title.upper())
        if match:
            explicit_values.extend(re.split(r"[,\s]+", match.group(1)))
    ...
    explicit_normalized: set[str] = set()
    for value in explicit_values:
        if not value:
            continue
        try:
            explicit_normalized.add(normalize_symbol(value))
        except ValueError:
            continue
    ...
    return normalized[:200], explicit_normalized
...
def main() -> int:
    ...
    requested, explicit = parse_requested_tickers(args)
    explicit_failures: list[str] = []

    for symbol in requested:
        try:
            payload = build_one(client, symbol)
            resolved_symbol = payload.get("symbol") or symbol
            write_json(MARKET_DIR / f"{resolved_symbol}.json", payload)
            resolved_note = "" if resolved_symbol == symbol else f" (resolved to {resolved_symbol})"
            print(f"{symbol}{resolved_note}: {payload['type']} / {len(payload['history']['dates'])} days / OK")
        except Exception as exc:
            failures.append(f"{symbol}: {exc}")
            if symbol in explicit:
                explicit_failures.append(f"{symbol}: {exc}")
    ...
    if explicit_failures:
        # A person explicitly asked for these tickers (via --ticker or a
        # "[data] TICKER" issue) -- silently reporting "success" here would
        # tell them the request worked when it didn't. Fail the job instead
        # so the "issue failed" comment fires and points them at the log.
        print("\nExplicitly requested tickers failed:", file=sys.stderr)
